...
public class QFileMetadataQuerydsl extends com.querydsl.sql.RelationalPathBase<FileMetadataQuerydsl>{
    ...
    public void addMetadata() {
        ...
        addMetadata(mimeType, ColumnMetadata.named("mimeType").withIndex(4).ofType(Types.VARCHAR).withSize(255).notNull());
        addMetadata(fileSize, ColumnMetadata.named("fileSize").withIndex(5).ofType(Types.DECIMAL).withSize(19).notNull());
        addMetadata(creationDate, ColumnMetadata.named("creationDate").withIndex(6).ofType(Types.TIMESTAMP).withSize(19).notNull());
